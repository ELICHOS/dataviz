diffi.na<-c( "Plus difficile aujourd'hui",
          "Aussi difficile aujourd'hui",
          "Ne sait pas",
          "Moins difficile aujourd'hui")
nombri.na<-c("Aussi nombreuses", "Moins nombreuses","Ne sait pas","Plus nombreuses")[c(4, 1, 3, 2)]
futuri.na<-c("Ne sait pas",   "S'accroitre" ,  "Se réduire" ,   "Se stabiliser")[c(2, 4, 1, 3)]
nonoui.na<-c("Non", "Ne sait pas", "Oui")
stabi.na<-c("Est restée stable", "Ne sait pas",       "S'est améliorée",   "S'est dégradée" )[c(4, 1, 2, 3)]
quali.na<-c("Ne sait pas",            "Se sont améliorées",
...
nesp<-gray(level = 0.2)
unkn<-gray(level = 0.5)
c(nega, stab, nesp, posi)->nombri
names(nombri)<-nombri.na
c(nega, stab, nesp, posi)->stabi
names(stabi)<-stabi.na
